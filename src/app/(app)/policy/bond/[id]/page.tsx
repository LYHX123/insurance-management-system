...
export default async function BondRecordDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = await prisma.policyRecord.findUnique({
    where: { id, category: "BOND", deletedAt: null },
    include: {
  ...
  return <BondDetailView detail={detail} customers={customers} />;
}
