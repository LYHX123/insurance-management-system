...
export default async function WorkPermitRecordDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = await prisma.policyRecord.findUnique({
    where: { id, category: "WORK_PERMIT", deletedAt: null },
    include: {
  ...
  if (!record || !record.workPermitDetail) notFound();
  ...
  return <WorkPermitDetailView detail={detail} customers={customers} isAdmin={isAdmin(session.user)} />;
}
