...
import {
  ...
  importSelectedNonMotorRowsAction,
  type ImportResultReport,
  type ImportBatchSummary,
} from "@/app/(app)/policy/non-motor/import/actions";

// Same interaction model as Motor's ImportPreviewTable (customer resolution
// panels, possible-match panel, duplicate-group panel, selection controls,
// row table, confirm modal, result screen) — deliberately not shared as one
// generic component (Motor's version is typed tightly to its own 21-column
// ImportPreviewRowData and Motor-specific actions; see this phase's research
// notes on why the codebase's convention is to duplicate per category rather
// than force a premature shared abstraction). The row table itself has far
// fewer columns since Non-Motor's standard format has no
// balance/commission/payment columns to import.
export type NonMotorImportPreviewRowData = {
  id: string;
...
const STATUS_TONE: Record<NonMotorImportPreviewRowData["status"], "neutral" | "brand" | "success" | "warning" | "danger"> = {
  READY: "success",
  WARNING: "warning",
...
const MATCH_TONE: Record<NonMotorImportPreviewRowData["customerMatchStatus"], "neutral" | "brand" | "success" | "warning" | "danger"> = {
  MATCHED: "success",
  POSSIBLE: "warning",
  MANUAL: "brand",
  UNMATCHED: "danger",
...
function isRowEligible(r: NonMotorImportPreviewRowData): boolean {
  return (
    r.includeInImport &&
    r.status !== "ERROR" &&
...
export function ImportPreviewTable({
  ...
  rows: NonMotorImportPreviewRowData[];
  ...
  const [statusFilter, setStatusFilter] = useState<"ALL" | NonMotorImportPreviewRowData["status"]>("ALL");
  ...
  const statusLabel: Record<NonMotorImportPreviewRowData["status"], string> = {
  ...
  const matchLabel: Record<NonMotorImportPreviewRowData["customerMatchStatus"], string> = {
  ...
  const counts = useMemo(() => {
    const c: Record<NonMotorImportPreviewRowData["status"], number> = {
  ...
  const handleImportSelected = async () => {
    setConfirming(true);
    setConfirmError(null);
    const result = await importSelectedNonMotorRowsAction(batchId, confirmedRowIds);
    setConfirming(false);
    if (!result.success) {
    ...
    return (
      <div className="flex flex-col gap-section">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={() => router.push("/policy/non-motor")}>{t.policy.importDone}</Button>
        </div>
      </div>
    );
  }
        ...
        <Table className="min-w-[1500px]">
          <thead>
            <tr>
              ...
              <th>{t.policy.importColPolicyNumber}</th>
              ...
              <th>{t.policy.importColProject}</th>
              <th>{t.policy.importColStatus}</th>
              <th>{t.policy.importColWarnings}</th>
            </tr>
          </thead>
          <tbody>
            {pageRows.length === 0 && <TableEmpty colSpan={15}>{t.policy.noRecordsNonMotor}</TableEmpty>}
            {pageRows.map((r) => {
              ...
              return (
                <tr key={r.id}>
                  ...
                  <td className="text-zinc-500">{r.policyNumber || "—"}</td>
                  ...
                  <td className="text-zinc-500">{r.projectNameRaw || "—"}</td>
                  <td>
