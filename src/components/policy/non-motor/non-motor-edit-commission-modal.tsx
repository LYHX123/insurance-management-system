...
// Phase 3A: reuses the exact same shared, category-agnostic
// updateCommissionAction as Motor (see that action's doc comment) — no
// Non-Motor-specific commission action or DB field exists, or should.
import { updateCommissionAction } from "@/app/(app)/policy/motor/actions";
import type { NonMotorDetail } from "@/components/policy/types";
...
// Duplicated from motor/edit-commission-modal.tsx only because that
// component's prop type is narrowly MotorDetail (Motor-specific fields
// TypeScript would otherwise require) — the underlying behavior, action,
// and validation are identical, never a second commission architecture.
export function NonMotorEditCommissionModal({
  detail,
  onClose,
  onSuccess,
}: {
