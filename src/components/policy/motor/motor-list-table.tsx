...
export function MotorListTable({ records }: { records: MotorListRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((r) => {
      ...
      const matchesExpiryTo = !expiryTo || expiry <= expiryTo;
      const matchesOutstandingClient = !outstandingClientOnly || Number(r.clientBalance) > 0;
      const matchesOutstandingInsurer = !outstandingInsurerOnly || Number(r.insurerBalance) > 0;
      return (
        matchesTerm &&
        matchesCustomer &&
        matchesType &&
        ...
        matchesExpiryFrom &&
        matchesExpiryTo &&
        matchesOutstandingClient &&
        matchesOutstandingInsurer
      );
    });
  }, [
    records,
    ...
    <div className="flex flex-col gap-section">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Input
          type="date"
          value={expiryFrom}
          onChange={(e) => resetToFirstPage(setExpiryFrom)(e.target.value)}
          className="w-auto"
          aria-label={t.policy.filterExpiryFrom}
        />
        <Input
          type="date"
          value={expiryTo}
          onChange={(e) => resetToFirstPage(setExpiryTo)(e.target.value)}
          className="w-auto"
          aria-label={t.policy.filterExpiryTo}
        />
        <label className="flex items-center gap-1.5 text-sm text-zinc-600">
          <input
            type="checkbox"
            checked={outstandingClientOnly}
            onChange={(e) => resetToFirstPage(setOutstandingClientOnly)(e.target.checked)}
        ...
        <label className="flex items-center gap-1.5 text-sm text-zinc-600">
          <input
            ...
            onChange={(e) => resetToFirstPage(setOutstandingInsurerOnly)(e.target.checked)}
            className="h-4 w-4 rounded border-zinc-300"
          />
