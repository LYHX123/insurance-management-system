...
export function MotorDetailView({
  ...
  // ?tab=financial. Now kept in sync afterward too (Phase 8 Part 6.5) — the
  // tab buttons below write the current tab back into the URL via replace,
  // so refresh/share/returnTo all restore the tab actually being viewed.
  ...
  return (
    <div className="flex flex-col gap-section">
      <div>
        <SmartBackLink fallbackHref="/policy/motor" label={t.policy.backToList} />
        <PageHeader
          title={
      ...
      {tab === "overview" && (
        <>
          <MotorOverviewTab detail={detail} customers={customers} canEdit={canEdit} canDelete={canDelete} />
