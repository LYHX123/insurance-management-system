...
// Reuses the exact same shared, category-agnostic updateCommissionAction as
// Motor/Non-Motor (see that action's doc comment) — no Bond-specific
// commission action or DB field exists, or should.
import { updateCommissionAction } from "@/app/(app)/policy/motor/actions";
import type { BondDetail } from "@/components/policy/types";
...
// Duplicated from non-motor-edit-commission-modal.tsx only because that
// component's prop type is narrowly NonMotorDetail — the underlying
// behavior, action, and validation are identical, never a second commission
// architecture.
export function BondEditCommissionModal({
  detail,
  onClose,
  onSuccess,
}: {
