...
// every other category (see that action's doc comment) — no Work
// Permit-specific commission action or DB field exists, or should. Section
// 15 of this phase's spec permits Commission to remain available for Work
// Permit using the same PolicyRecord fields; no business rule found
// excluding it.
import { updateCommissionAction } from "@/app/(app)/policy/motor/actions";
import type { WorkPermitDetail } from "@/components/policy/types";
...
export function WorkPermitEditCommissionModal({
  detail,
  onClose,
  onSuccess,
}: {
