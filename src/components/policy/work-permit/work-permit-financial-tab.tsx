...
// Reused directly, unchanged — both modals are already category-agnostic
// (see their own source), calling the same
// addCustomerReceiptAction/addProviderPaymentAction every other category
// uses. AddPaymentModal's own internal labels are generic ("Amount",
// "Payment Method", etc.) — no Agent-specific wording is needed inside it.
...
import { WorkPermitEditCommissionModal } from "@/components/policy/work-permit/work-permit-edit-commission-modal";
import type { WorkPermitDetail, PolicyPaymentStatus } from "@/components/policy/types";
...
// Same underlying PolicyRecord.insurerCost / PolicyProviderPayment
// architecture as every other category's financial tab — only the labels
// differ (Agent Cost / Total Paid to Agent / Agent Balance / Agent Payment
// Status / Add Agent Payment / Agent Payment History instead of Insurer *).
// Agent Cost/payments never affect Client Balance — that calculation
// (customerPremium - totalReceived) is entirely independent, unchanged from
// every other category.
export function WorkPermitFinancialTab({ detail, canEdit }: { detail: WorkPermitDetail; canEdit: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card>
          <h2 className="section-title mb-4">{t.policy.agentCardTitle}</h2>
          <dl className="grid grid-cols-2 gap-3 text-sm">
            <div><dt className="text-secondary">{t.policy.agent}</dt><dd className="font-medium text-zinc-800">{detail.agent}</dd></div>
            <div><dt className="text-secondary">{t.policy.agentCost}</dt><dd className="font-medium text-zinc-800">{formatMoney(detail.insurerCost)}</dd></div>
            <div><dt className="text-secondary">{t.policy.agentTotalPaid}</dt><dd className="font-medium text-zinc-800">{formatMoney(detail.totalPaid)}</dd></div>
            <div><dt className="text-secondary">{t.policy.agentBalance}</dt><dd className="font-medium text-zinc-800">{formatMoney(detail.insurerBalance)}</dd></div>
            <div>
              <dt className="text-secondary">{t.policy.agentPaymentStatus}</dt>
          ...
          {canEdit && (
            <div className="mt-4 flex justify-end">
              <Button variant="secondary" onClick={() => setShowAddPayment(true)}>
                <Plus size={16} />
                {t.policy.addAgentPayment}
              </Button>
            </div>
          )}
          <div className="mt-4">
            <h3 className="mb-2 text-sm font-medium text-zinc-700">{t.policy.agentPaymentHistory}</h3>
            <TableWrap scroll>
              <Table>
                <thead>
                  <tr>
                ...
                <tbody>
                  {detail.providerPayments.length === 0 && <TableEmpty colSpan={4}>{t.policy.noAgentPaymentsYet}</TableEmpty>}
