...
// Phase 8.1 Part 4 — Motor Claim list: search/customer/insurer/nature/
// progress/status/dateFrom/dateTo/page move from local useState into
// URL-persisted state (useUrlListState), plus a server-filtered customerId
// arriving only via a "View All Motor Claims" deep-link (never a dropdown).
// No component-rendering test infrastructure in this project (no RTL/jsdom
// — see vitest.config.ts's node environment), so — consistent with
// documentTableLayout.test.ts's existing convention — this asserts on
// source text rather than rendered DOM output.

const tableSource = readFileSync(join(__dirname, "..", "motor-claim-table.tsx"), "utf8");
const detailSource = readFileSync(join(__dirname, "..", "motor-claim-detail.tsx"), "utf8");

describe("Motor Claim table — URL-persisted list state", () => {
  it("uses the shared useUrlListState hook for every filter + page, not local useState", () => {
    ...
    expect(tableSource).toMatch(/useUrlListState\(MOTOR_CLAIM_LIST_DEFAULTS\)/);
  ...
  it("defaults cover every existing filter field plus page and customerId", () => {
    const defaultsMatch = tableSource.match(/const MOTOR_CLAIM_LIST_DEFAULTS = (\{[\s\S]*?\});/);
    expect(defaultsMatch).not.toBeNull();
    const defaultsLiteral = defaultsMatch![1];
    for (const field of ["search", "customer", "insurer", "nature", "progress", "status", "dateFrom", "dateTo", "page", "customerId"]) {
  ...
  it("search is debounced (no immediate flag) but resets page; every other control writes immediately and resets page", () => {
    ...
    for (const field of ["customer", "insurer", "nature", "progress", "status", "dateFrom", "dateTo"]) {
  ...
  it("the View link carries returnTo so the current filtered URL survives navigating into the detail page", () => {
    ...
    expect(tableSource).toMatch(/\/task\/motor-claim\/\$\{c\.id\}\?returnTo=\$\{encodeURIComponent\(returnTo\)\}/);
  });
});

describe("Motor Claim detail — delete preserves the filtered return URL", () => {
  it("resolves the same smart-back href used by SmartBackLink instead of a hardcoded bare URL", () => {
    ...
    expect(detailSource).toMatch(/const backHref = useSmartBackHref\("\/task\/motor-claim"\);/);
    expect(detailSource).toMatch(/router\.replace\(backHref\);/);
    // The old hardcoded, filter-stripping replace must be gone.
    expect(detailSource).not.toMatch(/router\.replace\("\/task\/motor-claim"\);/);
  });
