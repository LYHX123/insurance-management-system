...
import { previewFire } from "@/lib/insuranceCalculations/clientPreview";
import type { FireDraft } from "@/components/quotations/sectionDrafts";
...
export function FireSection({
  ...
  draft: FireDraft;
  onChange: (patch: Partial<FireDraft>) => void;
}) {
  const { t } = useLocale();

  const totals = useMemo(() => previewFire(draft), [draft]);

  return (
    <div>
      <div className="form-grid">
        <FormField label={t.quotations.propertyValue}>
          <MoneyInput
        ...
        <FormField label={t.quotations.rawMaterialValue}>
          <MoneyInput
            value={draft.rawMaterialValue}
            onChange={(v) => onChange({ rawMaterialValue: v })}
          />
        </FormField>
        <FormField label={t.quotations.goodsInStockValue}>
          <MoneyInput
            value={draft.goodsInStockValue}
            onChange={(v) => onChange({ goodsInStockValue: v })}
          />
        </FormField>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 rounded-control bg-zinc-50 p-3 text-sm sm:grid-cols-4">
        <div>
          <div className="text-secondary">{t.quotations.totalSumInsured}</div>
          <div className="font-medium text-zinc-800">{formatMoney(totals.totalSumInsured)}</div>
        </div>
        <div>
          <div className="text-secondary">{t.quotations.basicPremium}</div>
          <div className="font-medium text-zinc-800">{formatMoney(totals.basicPremium)}</div>
        </div>
        <div>
          <div className="text-secondary">{t.quotations.earthquakeLoadingAmount}</div>
          <div className="font-medium text-zinc-800">{formatMoney(totals.earthquakeLoadingAmount)}</div>
        </div>
        <div>
          <div className="text-secondary">{t.quotations.floodLoadingAmount}</div>
          <div className="font-medium text-zinc-800">{formatMoney(totals.floodLoadingAmount)}</div>
        </div>
        <div>
